...
sed -i -E "s/TempleEx\.version = \{major = [0-9]+, minor = [0-9]+, patch = [0-9]+\}/TempleEx.version = {major = $VMAJ, minor = $VMIN, patch = $VPAT}/" TempleEx-full.lua
sed -i -E "s/TempleApi\.version = \{major = [0-9]+, minor = [0-9]+, patch = [0-9]+\}/TempleApi.version = {major = $VMAJ, minor = $VMIN, patch = $VPAT}/" TempleEx-full.lua
sed -i -E "s/Git\.currentVersion = \"[0-9.]+\"/Git.currentVersion = \"$VERSION\"/" TempleEx-full.lua

echo "Built TempleEx-full.lua ($(wc -c < TempleEx-full.lua) bytes) v$VERSION"

# Update bootloader version
if [ -f "TempleEx.lua" ]; then
    sed -i "s/local BOOTLOADER_VERSION = \"[^\"]*\"/local BOOTLOADER_VERSION = \"$VERSION\"/" TempleEx.lua
    echo "Updated TempleEx.lua bootloader version to v$VERSION"
fi
